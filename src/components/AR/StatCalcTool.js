...
const gearStatChange = aostats.gearStatChange

const cap = 300

const defaultBonusStats = 157
...
class StatCalcTool extends Component {
  ...
  constructor(props) {
    ...
    let initState = {
      gear: 'B',
      cpus: Object.values(allItems)
        .filter(item => item.kind === 26)
  ...
  render() {
    ...
    const cpuData = cpus.find(eachCpu => eachCpu.id === Number(cpu))
